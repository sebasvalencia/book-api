...
def test_create_book():
    ...
    response = client.post("/books/", json=book_data.model_dump())
    assert response.status_code == 200
...
def test_update_book():
    ...
    response = client.put(f"/books/{book_id}", json=book_data.model_dump())
    assert response.status_code == 200
...
def test_update_book_internal_error():
    ...
    try:
        response = client.put(f"/books/{book_id}", json=book.model_dump())
